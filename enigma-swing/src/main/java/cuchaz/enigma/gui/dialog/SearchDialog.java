...
public class SearchDialog {
	...
	/**
	 * Updates the list of class names based on the current search query.
	 */
	private void updateList() {
		...
		Runnable updater = new Runnable() {
			@Override
			public void run() {
				if (!SearchDialog.this.dialog.isVisible()) {
					return;
				}
				...
				while (count > 0 && !queue.isEmpty()) {
					var o = queue.remove();
					SearchDialog.this.classListModel.insertElementAt(o.e, o.idx);
					count--;
				}
