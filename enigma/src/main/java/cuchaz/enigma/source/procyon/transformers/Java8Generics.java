	...
	static class Visitor extends DepthFirstAstVisitor<Void,Void> {
		...
		public Void visitCastExpression(CastExpression node, Void data) {
			...
			if (typeReference.isArray() && typeReference.getElementType().isGenericType()) {
				...
				if (typeReference instanceof IGenericInstance instance && instance.getTypeArguments().stream().anyMatch(TypeReference::isWildcardType)) {
					doReplace = true;
				} else if (target instanceof InvocationExpression invocationExpression
						&& invocationExpression.getTarget() instanceof MemberReferenceExpression expression && !expression.getTypeArguments().isEmpty()) {
					expression.getTypeArguments().clear();
					doReplace = true;
				}
			}
