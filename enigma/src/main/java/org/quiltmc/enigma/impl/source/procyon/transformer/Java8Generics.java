package org.quiltmc.enigma.impl.source.procyon.transformer;
...
public class Java8Generics implements IAstTransform {
	...
	static class Visitor extends DepthFirstAstVisitor<Void, Void> {
		...
		public Void visitObjectCreationExpression(ObjectCreationExpression node, Void data) {
			...
			if (type instanceof SimpleType simpleType && !simpleType.getTypeArguments().isEmpty()) {
		...
		public Void visitCastExpression(CastExpression node, Void data) {
			...
			if (typeReference.isArray() && typeReference.getElementType().isGenericType()) {
				...
				if (typeReference instanceof IGenericInstance genericInstance && genericInstance.getTypeArguments().stream().anyMatch(TypeReference::isWildcardType)) {
					doReplace = true;
				} else if (target instanceof InvocationExpression invocationExpression
						&& invocationExpression.getTarget() instanceof MemberReferenceExpression referenceExpression && !referenceExpression.getTypeArguments().isEmpty()) {
					referenceExpression.getTypeArguments().clear();
					doReplace = true;
				}
			}
